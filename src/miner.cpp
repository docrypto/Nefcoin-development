...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("NEFMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("NEFMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("NEFMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nef-miner");
...
////////////|| !masternodeSync.IsSynced()
        ...
        LogPrintf("Running NEFMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
